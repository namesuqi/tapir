...
class ProtocolPilot(object):
    ...
    def generate_protocol_message(protocol_object, protocol_type):
        ret_string = protocol_object.protocol_type
        try:
            tmp_sub_type = (protocol_object.protocol_opt << 7 | protocol_object.protocol_iack << 6 |
                            protocol_object.protocol_rsv << 5) & 0xf0
        except Exception, e:
            print e.message
        tmp_sub_type |= (protocol_object.protocol_sub_type & 0x0f)
        ret_string += binascii.b2a_hex(struct.pack("!B", tmp_sub_type))
        ret_string += protocol_object.protocol_win_size
        ret_string += binascii.b2a_hex(struct.pack("!I", protocol_object.protocol_timestamp))
        ret_string += binascii.b2a_hex(struct.pack("!I", protocol_object.protocol_time_delay))
        ret_string += binascii.b2a_hex(struct.pack("!H", protocol_object.protocol_seqno))
        ret_string += binascii.b2a_hex(struct.pack("!H", protocol_object.protocol_ackno))
        if protocol_type == PROTOCOL_DCCP_SYNC:
            ret_string += protocol_object.protocol_data.data_peerid
        elif protocol_type == PROTOCOL_DCCP_SYNACK:
            pass
        elif protocol_type == PROTOCOL_DCCP_DATA:
            if protocol_object.protocol_data.data_cccp_obj.protocol_type == PROTOCOL_CCCP_PUSH_STREAM_REQ:
                ret_string += binascii.b2a_hex(struct.pack(
                    "!B", protocol_object.protocol_data.data_cccp_obj.protocol_type))
                ret_string += binascii.b2a_hex(struct.pack(
                    "!H", protocol_object.protocol_data.data_cccp_obj.protocol_data.request_id))
                ret_string += protocol_object.protocol_data.data_cccp_obj.protocol_data.peer_id
                ret_string += protocol_object.protocol_data.data_cccp_obj.protocol_data.file_id
                tmp_array = bytearray(256)
                tmp_array[:len(protocol_object.protocol_data.data_cccp_obj.protocol_data.file_url)] = \
                    protocol_object.protocol_data.data_cccp_obj.protocol_data.file_url
                ret_string += binascii.b2a_hex(str(tmp_array))
                ret_string += binascii.b2a_hex(struct.pack(
                    "!H", protocol_object.protocol_data.data_cccp_obj.protocol_data.cppc_number))
            elif protocol_object.protocol_data.protocol_type == PROTOCOL_CCCP_PUSH_STREAM_RSP:
                pass
            elif protocol_object.protocol_data.protocol_type == PROTOCOL_CCCP_PUSH_PIECE_DATA:
                pass
            elif protocol_object.protocol_data.protocol_type == PROTOCOL_CCCP_PUSH_STREAM_FIN:
                ret_string += binascii.b2a_hex(struct.pack(
                    "!B", protocol_object.protocol_data.data_cccp_obj.protocol_type))
                ret_string += binascii.b2a_hex(struct.pack(
                    "!H", protocol_object.protocol_data.data_cccp_obj.protocol_data.request_id))
                ret_string += protocol_object.protocol_data.data_cccp_obj.protocol_data.file_id
                ret_string += binascii.b2a_hex(struct.pack(
                    "!B", protocol_object.protocol_data.data_cccp_obj.protocol_data.status))
        elif protocol_type == PROTOCOL_DCCP_ACK:
            ret_string += binascii.b2a_hex(struct.pack("!B", protocol_object.protocol_data.data_whyack))
        elif protocol_type == PROTOCOL_DCCP_FIN:
            ret_string += binascii.b2a_hex(struct.pack("!B", protocol_object.protocol_data.data_code))
    ...
    def parse_dccp_packet(packet_data):
        ...
        if protocol_type == PROTOCOL_HEAD_DCCP:
            ...
            protocol_data.protocol_type = PROTOCOL_HEAD_DCCP
            protocol_data.protocol_opt = (int(binascii.b2a_hex(data_bytes[1]), 16) & 0x80) >> 7
            protocol_data.protocol_iack = (int(binascii.b2a_hex(data_bytes[1]), 16) & 0x40) >> 6
            protocol_data.protocol_rsv = (int(binascii.b2a_hex(data_bytes[1]), 16) & 0x30) >> 4
            protocol_data.protocol_sub_type = int(binascii.b2a_hex(data_bytes[1]), 16) & 0x0f
            protocol_data.protocol_win_size = binascii.b2a_hex(data_bytes[2:4])
            protocol_data.protocol_timestamp = long(binascii.b2a_hex(data_bytes[4:8]), 16)
            protocol_data.protocol_time_delay = long(binascii.b2a_hex(data_bytes[8:12]), 16)
            protocol_data.protocol_seqno = int(binascii.b2a_hex(data_bytes[12:14]), 16)
            protocol_data.protocol_ackno = int(binascii.b2a_hex(data_bytes[14:16]), 16)
            if protocol_data.protocol_sub_type == PROTOCOL_DCCP_SYNC:
                return protocol_data, binascii.b2a_hex(data_bytes[-2:])
            elif protocol_data.protocol_sub_type == PROTOCOL_DCCP_SYNACK:
                ...
                protocol_data.protocol_data = temp_data
                return protocol_data, tmp_chksum
            elif protocol_data.protocol_sub_type == PROTOCOL_DCCP_DATA:
                ...
                if temp_sub_type == PROTOCOL_CCCP_PUSH_STREAM_REQ:
                    ...
                    protocol_data.protocol_data = cccp_data
                    return protocol_data, tmp_chksum
                    ...
                    protocol_data.protocol_data = cccp_data
                    return protocol_data, tmp_chksum
            elif protocol_data.protocol_sub_type == PROTOCOL_DCCP_ACK:
                ...
                protocol_data.protocol_data = temp_data
                return protocol_data, tmp_chksum
            elif protocol_data.protocol_sub_type == PROTOCOL_DCCP_FIN:
