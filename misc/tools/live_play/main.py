...
# author: zengyuetian
"""
if start hang, you can run deploy and then restart
in case that start play before player get deployed
"""
...
CHANNEL_URL = "live_flv/user/xmtv?url=http://pullsdk.test.live.00cdn.com/live/stream999.flv"
...
class RemoteDeployer(object):
    ...
    def deploy_folder(self, local_dir, remote_dir, kill_proc=None):
        ...
        ssh.exec_command("rm -rf {dir}".format(dir=remote_dir))
        ssh.exec_command("mkdir -p {dir}".format(dir=remote_dir))
        ssh.close()
...
class RemoteNode(object):
    ...
    def start_play(self):
        ...
        for i in range(self.sdk_nums):
            ...
            if SUPPORT_MULTI_CHANNELS:
                channel_url = self.url[:-4] + str(start+i) + self.url[-4:]
            else:
                channel_url = self.url
            url = "http://127.0.0.1:{port}/{url}".format(port=port, url=channel_url)
            command = "nohup python {player}/flv_play.py {url} > /dev/null 2>&1 &".format(
                player=REMOTE_PLAYER_PATH, url=url)
            print command
            time.sleep(0.5)
            ssh.exec_command(command)
        ssh.close()
    ...
    def start_sdk(self):
        ...
        for i in range(self.sdk_nums):
            port = i * SDK_PORT_STEP + SDK_PORT_START
            print "Start SDK for port {port}".format(port=port)
            ssh.exec_command("mkdir -p {path}/{dir}".format(path=REMOTE_SDK_PATH, dir=i))
            ssh.exec_command("cp {0} {1}/{2}/{3}".format(REMOTE_SDK_FILE, REMOTE_SDK_PATH, i, SDK_FILE))
            time.sleep(0.5)
            p2pclient = "ulimit -c unlimited && cd {0}/{1} && nohup ./{2}".format(REMOTE_SDK_PATH, i, SDK_FILE)
    ...
    def restart_sdk(self):
        ...
        for i in range(self.sdk_nums):
            ...
            p2p_client = "ulimit -c unlimited && cd {0}/{1} && nohup ./{2}".format(REMOTE_SDK_PATH, i, SDK_FILE)
...
def collect_p2p_test():
    while True:
        try:
            ...
            print "Alive sdk average download rate {0} kbps".format(sum(download_rate_list) / len(non_zero_download_list))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        if sys.argv[1] == "start":
            ...
            tester.deploy_test()
        elif sys.argv[1] == "stop_live_push":  # stop sdk, stop play
            tester.stop_live_push_test()
