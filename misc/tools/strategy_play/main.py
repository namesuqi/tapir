...
# path definition
REMOTE_SDK_PATH = '/home/admin/group'
REMOTE_PLAYER_PATH = '/home/admin/group'
REMOTE_LOG_CONFIG_PATH = '/home/admin/group/conf'
...
class RemoteNode(object):
    ...
    def start_play(self):
        ...
        ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        ssh.connect(self.ip, SSH_PORT, self.username, self.password)
        command = "cd {player} ; nohup python {player}/local_main.py --num {num} --url {url} > local_main.log 2>&1 &".format(
                player=REMOTE_PLAYER_PATH, num=self.sdk_nums, url=self.url)
        ssh.exec_command(command)
        print command
        time.sleep(1)
        ssh.close()
...
class Tester(object):
    ...
    @staticmethod
    def print_help():
        print "Please use control type: [start|stop|core|sdk|player]"
